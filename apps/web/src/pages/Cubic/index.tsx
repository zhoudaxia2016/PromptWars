...
export default function Cubic() {
  ...
  const handleQuickScramble = () => {
    const scrambleStr = generateScramble(20);
    setScramble(scrambleStr);
    const formula = normalizeFormula(scrambleStr);
  ...
  const stickerClass = (letter: string) => {
    const key = letter === '?' ? 'u' : letter.toLowerCase();
    return `${s.sticker} ${(s as Record<string, string>)[key] || ''}`;
  };

  return (
    <div className={s.cubicPage}>
      <div className={s.topBar}>
        <Link to="/" className={s.backLink}>← 返回</Link>
        <div id="info" className={s.infoBadge}>
          🎨 <span className={s.infoAccent}>标准魔方</span> · 前红 后橙 右绿 左蓝 上黄 下白
        </div>
      </div>

      <div className={s.topPanel}>
        <div className={s.panelRow}>
          <button className={`${s.actionBtn} ${s.resetBtn}`} onClick={handleReset}>🔄 恢复魔方</button>
          <button className={s.actionBtn} onClick={handleUndo} disabled={moveCount === 0}>↩️ 撤销</button>
          <button className={s.actionBtn} onClick={handleResetCamera}>📷 复位视角</button>
          <button className={s.actionBtn} onClick={handleGenerateScramble}>🎲 生成WCA打乱</button>
          <button className={s.actionBtn} onClick={handleQuickScramble}>⚡ 快速打乱</button>
          <button className={s.actionBtn} onClick={() => setCfopOpen(true)}>📋 CFOP 公式</button>
        </div>
        <div
          className={`${s.scrambleDisplay} ${!scramble ? s.placeholder : ''}`}
          onClick={handleScrambleClick}
        >
          {scramble ? <><span className={s.scrambleAccent}>打乱公式</span> {scramble}</> : '点击生成打乱公式'}
        </div>
        <div className={s.formulaRow}>
          ...
          />
          <button className={s.actionBtn} onClick={handleExecuteFormula}>执行公式</button>
        </div>
      </div>
      ...
      {stickerState && (
        <div className={s.stickerPanel}>
          <div className={s.stickerPanelTitle}>当前状态</div>
          <div className={s.stickerFace} style={{ gridColumn: 2, gridRow: 2 }}>
            {flipRows(stickerState.U ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <div className={s.stickerFace} style={{ gridColumn: 1, gridRow: 3 }}>
            {(stickerState.L ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <div className={s.stickerFace} style={{ gridColumn: 2, gridRow: 3 }}>
            {(stickerState.F ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <div className={s.stickerFace} style={{ gridColumn: 3, gridRow: 3 }}>
            {(stickerState.R ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <div className={s.stickerFace} style={{ gridColumn: 4, gridRow: 3 }}>
            {(stickerState.B ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <div className={s.stickerFace} style={{ gridColumn: 2, gridRow: 4 }}>
            {flipRows(stickerState.D ?? []).map((letter, i) => (
              <div key={i} className={stickerClass(letter)} />
            ))}
          </div>
          <button className={`${s.copyBtn} ${s.mt6}`} onClick={copyState54}>复制贴纸状态</button>
          <button className={`${s.copyBtn} ${s.mt2}`} onClick={copyAIPrompt}>复制 AI 还原提示词</button>
        </div>
      )}
      ...
      <div className={s.controlPanel}>
        <div className={s.controlButtons}>
          <div className={s.controlRow}>
            {(['U', 'D', 'L', 'R', 'F', 'B'] as const).map((face) => (
              <button key={face} className={s.controlBtn} data-face={face} onClick={() => handleRotate(face)}>
                {face}
              </button>
            ))}
          </div>
        ...
        <div className={s.controlHint}>
          <div className={s.hintText}>🖱️ 鼠标拖拽自由视角 | 滚轮缩放 | 点击公式执行打乱</div>
          <div>
            <label className={s.speedLabel}>
              单次旋转时间 <span className={s.speedValue}>{rotationSpeed} ms</span>
            </label>
            <input
              type="range"
              min={100}
      ...
      {cfopOpen && (
        <div className={s.cfopOverlay} onClick={() => setCfopOpen(false)}>
          <div className={s.cfopContent} onClick={(e) => e.stopPropagation()}>
            <div className={s.cfopHeader}>
              <div className={s.cfopTabs}>
                {(['OLL', 'PLL', 'F2L'] as const).map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setCfopTab(tab)}
                    className={`${s.cfopTab} ${cfopTab === tab ? s.active : ''}`}
                  >
                    {tab}
                  </button>
                ))}
              </div>
              <button onClick={() => setCfopOpen(false)} className={s.cfopClose}>×</button>
            </div>
            <div className={s.cfopList}>
              {cfopList.map((item) => (
                <div
                  key={item.id}
                  className={s.cfopItem}
                ...
                >
                  <span className={s.formulaId}>{item.id}</span> {item.name}: {item.formula}
                </div>
              ))}
            </div>
          </div>
